...
def get_last_sync_time():
    """Get the last sync time from file or return a default value."""
...
def export_biometric_data_and_continue(last_sync_time):
    """Export biometric data for the date and continue the process."""
    ...
    # Save attendance logs to a file
    try:
    ...
    for i, log in tqdm(enumerate(data_to_export), total=total_logs, desc="Sending logs", unit="log"):
        user_id = log['user_id']
    ...
    update_last_sync_time()  # Update the last sync time after successful export
...
if __name__ == "__main__":
    while True:
        try:
            ...
            last_sync_time = datetime.datetime.strptime(last_sync_time_str, '%Y-%m-%d %H:%M:%S') if last_sync_time_str else datetime.datetime.now() - datetime.timedelta(days=1)  # Default to 1 day ago if no sync time found

            # Run the export
            export_biometric_data_and_continue(last_sync_time)
        except Exception as e:
